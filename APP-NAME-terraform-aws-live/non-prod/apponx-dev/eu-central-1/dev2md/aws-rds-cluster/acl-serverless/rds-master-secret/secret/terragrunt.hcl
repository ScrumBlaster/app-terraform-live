...
inputs = {
  ...
  secret_description  = "${local.vars.inputs.env_vars.inputs.name}: AWS RDS master user password"
  ...
  mysql_username        = local.vars.inputs.rds_vars.inputs.rds_master_user
  mysql_password        = local.vars.inputs.rds_vars.inputs.rds_master_password

  # Specifies the number of days between automatic scheduled rotations of the secret. Set to 0 to disable rotation
